...
public interface UserPasswordServiceInterface 
{
  ...
  void create(UserLoginRequestVO record, UserToken token) throws ThingAppException;

  // we only want to return the password if it was explicitly requested
  // if it fails it will throw an exception
  @Transactional(rollbackFor = Throwable.class)
  void update(UserLoginRequestVO record, UserToken token) throws ThingAppException;
